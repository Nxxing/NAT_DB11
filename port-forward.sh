...
# Debian 11 Port Forward Script - Fixed Version
...
# Resolve hostname to IP address - FIXED: no logging inside function
resolve_hostname() {
    local hostname="$1"
...
# Detect network interface - FIXED: no logging inside function
detect_interface() {
    local interface="$1"
...
# Create port forwarding rules - FIXED: clean variable usage
create_forwarding_rules() {
    local dest_hostname="$1"
    local resolved_ip="$2"
    local port_start="$3"
    local port_end="$4"
    local interface="$5"
    ...
    log_info "Destination: $dest_hostname → $resolved_ip"
...
# Main function - FIXED: clean parameter handling
main() {
    ...
    # Resolve hostname - FIXED: separate logging from resolution
    log_info "Resolving hostname: $dest_hostname"
    local resolved_ip
    if ! resolved_ip=$(resolve_hostname "$dest_hostname"); then
        log_error "Failed to resolve hostname: $dest_hostname"
        echo ""
    ...
    log_info "✓ Resolved $dest_hostname → $resolved_ip"
    
    # Detect interface - FIXED: separate logging from detection
    local interface
    interface=$(detect_interface "$user_interface")
    ...
    create_forwarding_rules "$dest_hostname" "$resolved_ip" "$port_start" "$port_end" "$interface"
    save_configuration "$dest_hostname" "$resolved_ip" "$port_start" "$port_end" "$interface"
    create_manager
    
    # Verify and show results
    if verify_configuration; then
        show_summary "$dest_hostname" "$resolved_ip" "$port_start" "$port_end" "$interface"
    else
